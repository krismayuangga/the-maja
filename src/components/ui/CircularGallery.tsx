...
/* ─── Title (WebGL text label below each card) ─── */
...
class Title {
  ...
  constructor({
    ...
    void renderer; // keep param
  }
}

/* ─── Media (single gallery card) ─── */
...
class GalleryApp {
  ...
  onItemClick: ((item: any, index: number) => void) | undefined;
  isDragged: boolean;
  startX: number;
  ...
  onWheel(e: WheelEvent) {
    // Stop propagation so it doesn't trigger museum horizontal scroll
    e.stopPropagation();
  ...
  update() {
    // Auto-rotate: slowly advance target when not interacting
    if (!this.isDown && this.autoRotateSpeed) {
      this.scroll.target += this.autoRotateSpeed * 0.008;
    }
  ...
  addEventListeners() {
    ...
    // Scoped to container to avoid conflicting with museum horizontal scroll
