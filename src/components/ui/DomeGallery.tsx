...
interface DomeGalleryProps {
  ...
  grayscale?: boolean;
  /** When provided, clicking a tile calls this with the image src instead of the built-in enlarge */
  onItemClick?: (src: string) => void;
  /** Degrees per second for auto-rotation on Y axis. 0 = off. */
...
const getDataNumber = (el: HTMLElement, name: string, fallback: number) => {
  const attr =
    (el as any).dataset[name] ?? el.getAttribute(`data-${name}`);
  const n = attr == null ? NaN : parseFloat(attr);
  return Number.isFinite(n) ? n : fallback;
};

function buildItems(pool: (DomeImage | string)[], seg: number) {
...
export default function DomeGallery({
  ...
  useEffect(() => {
    ...
    const step = (now: number) => {
      ...
      lastTime = now;
      // Skip if dragging or an image is focused
